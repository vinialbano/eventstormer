...
| DB file location and lifecycle | `./data/eventstormer.db`, created and migrated on first store construction; `data/` is gitignored; `pnpm db:reset` deletes the file | ARCHITECTURE.md §7 states this path and `pnpm db:reset` | y — ARCHITECTURE.md |
