...
| DOCK (20) — proposal pill by kind | **met** | shared `dock/kind-label.ts` `kindLabel()` used by `FacilitatorDock.vue` (`:kind-label="kindLabel(card.blockKind)"`, was hard-coded `"EVENT"`) and `PendingDrawer.vue` (dup local map removed). Test: "labels a proposal card by its building-block kind, not always EVENT" (asserts `ACTOR`/`SYSTEM` pills, no `EVENT`). Pill colour unchanged (design-system gap → impeccable follow-up). |
