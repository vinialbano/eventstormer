...
| AD-035 | **One `Proposal` aggregate carries both a proposed building block and a proposed model change.** A second birth event `Model Change Proposed { proposalId, sessionId, contributionId, intent, at }` — `intent` a discriminated union `{ kind:'relation', relationKind, endpoints: BuildingBlockId[] } \| { kind:'pivotal', pivotalKind, target } \| { kind:'reword', target, newLabel }` — sits beside `Building Block Proposed`. Every disposition event (`Proposal Accepted`, `Operation Applied`, `Operation Application Failed`, `Proposal Rejected`, `Proposal Held`/`Unheld`, `Proposal Lapsed`) is reused unchanged; `replay` and the `review-proposal` accept handler branch on the birth event. A model-change `Accept Proposal` mints no `buildingBlockId`. A pre-accept edit is `Model Change Edited` (swaps an endpoint / target / `newLabel`; `intent.kind` fixed). | The disposition machine is identical for a model-change proposal — incl. `APPLY_FAILED` (cycle, withdrawn endpoint), unlike `Resolution`. A sibling aggregate duplicates ~90% of the lifecycle; overloading `Building Block Proposed` muddies "a proposal is a sticky" in every read model and test. domain-modeling: the log is the record — a second birth event is the smallest honest extension. | 2026-09-05 | Slice 5; all future facilitator-proposed operations |
| AD-036 | **The F04 reword-hold-back gate and the F07 pivotal-readiness gate are deterministic predicates over the board snapshot — the *predicates are `session-facilitation/domain/` vocabulary*, only their *application* is at the anticorruption seam (`mapTurn`); never a facilitator self-report.** `session-facilitation/domain/model-readiness.ts` exports `hasModelStructure(snapshot)` (≥ 1 `follows`/`causedBy` edge **OR** ≥ 1 pivotal mark) and `pivotalProposable(snapshot)` (≥ `PIVOTAL_MIN_PLACED_EVENTS = 5` placed domain events — a named constant, F07's "a handful … a long board"). Reword: a `propose-reword` track is `heldBack` unless `hasModelStructure` **or** its target was proposed earlier in the same contribution (the person is still authoring it — and accept ordering must apply the reword after that block's own proposal, else it lands `APPLY_FAILED`). A held track is stored (`heldBack: true`), births no `Proposal`, and is surfaced as a `sessionView` notice. Pivotal: a `propose-pivotal` track is dropped unless its target is a placed domain event **and** `pivotalProposable`; a below-threshold drop is **also** recorded as a `heldBack` track (symmetry with reword — the record shows the facilitator wanted it). Relation: dropped on unresolved/ambiguous endpoint label, wrong arity for `relationKind`, or equal endpoints. `boardState` is read from **one `readBoardSnapshot` taken at the same point as endpoint-label resolution — after the model call returns** (mirrors AD-025's post-`await` discipline). A held opportunity is genuinely lost unless the person rephrases — distinct from AD-020's `Proposal Parked` (that parks a live, still-reviewable proposal; this is the facilitator self-suppressing at interpret time, the `attribute-to-other-format` precedent). | "Model has structure" (AD-031) = relations/pivotal exist — precisely what Slice 3 introduced. A snapshot fold is testable with the held and released case side by side; a model self-report is not. The threshold is a product decision (F04/F07) and must be named in the language, not buried in infra. Mirrors the shipped `annotatesTargetId` label→id drop-on-unresolvable pattern. | 2026-09-05 | Slice 5 |
| AD-037 | **The JSON export serialises the Board snapshot + the `ArtifactSource` workshop record — not the operation log — and its round-trip is a pure `serialise` / `deserialise` pair verified by a `fast-check` property, with no HTTP import route.** `serialise` builds the object in fixed key order and sorts every array by a total key (blocks by id, `follows` by `(predecessor,successor)`, `causedBy` by `(cause,effect)`); it carries no quoted evidence (no contribution bodies, rationale, or evidence spans). The version stamp is **composite** — `{ boardPosition, sessionRecordPosition, renderedAt }` — because `ArtifactSource` fields (`scope`, `stakeholderCheck`, `chosenProblem`) are re-recordable without a board operation, so a board-position-only stamp would let two byte-different exports share a version. An empty board serialises as `boardPosition: 0` (the serialiser normalises `readBoardSnapshot`'s `-1`). `deserialise(serialise(x))` deep-equals `x` for every generated snapshot. `deserialise` bounds its input (block/edge count caps) — a hostile `ModelJson` is rejected, not expanded. | PRD F10: "a direct serialisation of the model … it round-trips: importing it reproduces the model exactly" — the model is the snapshot, not the log. Import as a user flow is F14 (multiplayer), out of v1; a pure function satisfies the AC literally and is the stronger determinism assertion. | 2026-09-05 | Slice 5 |
| AD-038 | **A relation / pivotal / `resolve` operation whose effect already holds is idempotent in `decide` — it returns `ok([])`, and `applyOperation` returns `ok` with the current board position for an empty decision.** `decide(sequence \| link-cause)` on an edge that already exists, `decide(mark-pivotal \| unmark-pivotal)` on a block already in that state, `decide(resolve)` on an already-resolved hot spot → `ok([])` rather than `err({ kind: 'already-related' \| 'already-resolved', classification: 'systemic' })`. `insert-between` / `unsequence` / `unlink-cause` / `unplace` on a missing edge stays a genuine `missing-edge` failure. | The `review-proposal` accept chain (AD-016) has a crash window: apply commits, the outcome-record commit is lost, the person re-accepts. For id-minting kinds the board's `duplicate-id` reconverges the retry to `APPLIED`; target-bearing relation ops had no such path, so the retry converged to a **wrong `APPLY_FAILED` with the edge already on the board** — board/`Proposal`-stream divergence in a reachable state, and a determinism-of-record defect in the F19 transcript. AD-022 already puts idempotency in the single-writer `decide`; this extends it from `stale-position` to already-satisfied effects. | 2026-09-05 | Slice 5 (`domain-model-capture` board decider) |
...
- **Phase / Task**: **Execute complete + Verifier PASS ✅ — PR [#91](https://github.com/vinialbano/eventstormer/pull/91) open against `main`.** All 27 tasks + Verifier + fix iteration 1 (`a15144b` SUM-09 equal-rank fixture; `8e6b0ab` FREW-03 pinned; `8de1dad` FREL-04 live smoke PASS). `validation.md`: **43/44 ACs clean**, sensor **6/6 killed**, only REL-02 (maintainer `gh` step) outstanding.
- **Branch**: `slice-5-artifacts-facilitator-tracks` off `main` (`0c8a6b4`), pushed. Commit range `2ffe038`…HEAD (~44 commits). `pnpm check` **1148 tests** (+162) + `pnpm build` + `pnpm test:e2e` (6, incl. the new `artifacts-and-relations.spec.ts`) all green.
- **FREL-04 live smoke: DONE** — `pnpm smoke:facilitation-schema` run 2026-09-07 (`8de1dad`, `claude-sonnet-5`): **PASS**, the assembled 11-member schema round-trips and the model produced all three new strands. `research/research-aisdk.md` "SLICE 5" has the LIVE RESULT. (Script now loads `.env.local`.)
- **Next step (maintainer)**: (1) the `gh issue` re-scope commands in the `7f3aaa7` commit body + `tasks.md` Execution Log (retitle #42, file "Slice 5b — facilitator eval + demo seed", update #43 blocked-by); (2) merge #91, then the changeset-version PR handles 0.6.0.
- **Slice 5b (follow-on) owns**: F11 eval suite + `pnpm seed` + the recorded walkthrough (recording dependency); the in-dock proposal card for relation/pivotal/reword proposals (server path complete + tested; the Vue `ProposalCard` renders building-block proposals only).
- **Slice 6 cleanup**: remove the dead `already-related` / `already-resolved` variants from the `domain-model-capture` board error union + consumers (AD-038 made them unreachable).
...
- **Verifier verdicts on the flagged items** (`validation.md` has the full report): (1) AD-038 slice-4 resolve-convergence change → **acceptable** (AD-038 explicitly scopes `resolve`; the modified slice-4 test still asserts strong outcomes; no other slice-4 test touched). (2) same-turn-reword drop → **WARN, now pinned** by `map.test.ts` (`8e6b0ab`) as intended v1 behaviour. (3) no dock proposal-card UI → **WARN, acceptable-as-scoped** (F04/F07 P1 = server-verified / UI-pending in Slice 5b). (4) `summary` string phrasing → NOTE. (5) live smoke → **blocking maintainer step**. Lessons L-012…L-015.
- **Original flags (context)**:
  1. AD-038's `resolve` idempotency changed shipped slice-4 behaviour — a competing `Resolution` on an already-resolved hot spot converges `APPLIED` (was `LAPSED` / `already-resolved`); the slice-4 `review-resolution` acceptance test was updated to match, and T27 removed the now-unreachable `'already-resolved'` entry from `LAPSE_REASONS` in `review-resolution/accept.ts` (the dead `already-resolved` member on the board error union in `domain-model-capture/domain/board/model.ts` was left in place — minimal-change).
  2. Batch-1 SPEC_DEVIATION in `map.ts` `rewordTrack` — a same-contribution reword whose target has no `BuildingBlockId` at interpret time is dropped at the seam (FREW-03 "not held" satisfied; the normal case is handled via `deriveTracks` birth order).
  3. T26 live smoke (`pnpm smoke:facilitation-schema`, FREL-04's manual half, out of CI per AD-027) is **PENDING a maintainer run** — no `ANTHROPIC_API_KEY` was available; `research/research-aisdk.md` records it as pending.
  4. Spec-precision gap: the `summary` string on the intent card / transcript proposal annotation is a chosen deterministic phrasing (`"<relationKind>: <labels →>"`, `"<pivotalKind>: <label>"`, `"reword: <old> → <new>"`) — the spec pins `disposition` + shape, not the human wording.
  5. T25 SPEC_DEVIATION: the scripted-facilitator e2e accepts the relation proposal via `POST /proposals/:id/accept` (the endpoint the dock calls) rather than a dock click — the dock renders no model-change proposal card in v1 (no Slice 5 app task builds one; folded into the eval/seed follow-on).
- **Plans**: `spec.md` / `design.md` / `tasks.md` hardened 2026-09-05 after a four-lens adversarial review; `tasks.md` Execution Log tracks batch progress (Batches 1–6).

---

### Slice 4 (prior feature — reference)

- **Feature**: `slice-4-hot-spots-close` (`.specs/features/slice-4-hot-spots-close/`) — GitHub **#41** · version target **0.5.0**
- **Phase / Task**: **Execute complete — Verifier PASS ✅. PR #90 open; addressing the automated review.** All 51 tasks + 4 Verifier fix commits + the PR #90 review-fix batch (`.specs/features/slice-4-review-fixes/`) on `slice-4-hot-spots-close`.
- **PR #90 review fixes (2026-09-03)**: B1 — `reconcileHotSpots` raise id now derived from `workshopId` + sweep key (deterministic retry → `duplicate-id`), not random. B2 — `finishClose` flips the `session_index` row **last**, after the lapse loop + sweep, so `reconcilePendingDerivations` retries a partial close. W1 — `Choose Problem` qualifies `firm` only after a *complete* stakeholder check (absent/incomplete → `provisional`). W2 — dropped the `SPEC_DEVIATION` framing from the `modelAffecting` comments. W3 + NOTE3 — `spec.md`/`context.md`/`design.md`/`tasks.md` now describe AD-032 choreography (not the abandoned AD-019 bus) and carry Verifier PASS; `detail?` field name aligned. Deferred: quick-win #1 (retry escalation — needs a migration), NOTE 1/2, W1 hard-reject tier.
- **Verifier (2026-09-03)**: iteration 0 FAIL (4 gaps) → fix iteration 1 (`9e8f100` scope-question close-sweep fix, `7b9084e`/`d2bc234`/`006fe48` the 3 missing tests, `046b87f` docs) → re-verify **PASS**: all 4 gaps closed, 6/6 sensor mutations killed (incl. the iteration-1 survivor), gate fully green, no test weakened (E2E assertion tightened). `validation.md` has the full "Re-verification (iteration 1)" section. Lessons L-010 (E2E papering over a spec defect) + L-011 (error-as-success branches need a forced-collision test) recorded as candidates.
- **Branch**: `slice-4-hot-spots-close` (not pushed). `pnpm check` (**984 tests**) + `pnpm build` + `pnpm test:e2e` (5) all green. `.changeset/slice-4-hot-spots-close.md` = `minor` (→ 0.5.0); `package.json` `version` untouched (ADR-009).
...
- **Next step**: push `slice-4-hot-spots-close`, open a PR against `main` (body: F08/F09/F18 per the changeset; link #41; note AD-032/033/034), close #41 on merge. The changeset-version PR handles 0.5.0.
- **Batch 7 deviations**: T50 touched `infrastructure/facilitator/map.ts` + `map.test.ts` (not in the task's file list) — `mapTurn` now runs `propose-resolution.hotSpotId` through `resolveBlockId` with a literal fallback, exactly as `annotatesTargetId` already works, so the scripted fixture can name a hot spot the run minted; zero behaviour change on the real-id path. Sensor row **M8** added to `docs/testing.md`.

**Slice-4 → Slice-6 reconciliation list** (record only — do NOT edit the docs now; folds into the S1-51b list below):
- **ADR-004** — the note that a hot spot's kind is "changeable by a reword-style operation" has **no frozen home**: the `v:1` operation union has no such operation, and Slice 4 makes the kind editable only *before* accept (on the review card). Either strike the note or file it as a `v:2` additive.
- **`Question Asked.kind`** gained `'stakeholder'` (T30, AD-034) — reconcile the `session-facilitation` canvas and the ubiquitous-language glossary (they list only `scope`/`phase`/`free`).
- **`#41 → #42` facilitator-track move** (AD-033) — ADR-010 / ARCHITECTURE.md slice-table still imply the relation / `mark-pivotal` proposal tracks + reword-hold-back gate are Slice 4; they are #42. Already commented on the issues; the doc reword is the Slice-6 pass.
- **Scope opening question never resolved** (see Verifier note above) — if the fix is "sweep skips `kind:'scope'`" or "scope-set answers the question", note it against the canvas `Session Closed` policy.
- **B5 notes**: acceptance test 44 at `interpret.test.ts:434`; `Choose`/`Skip` after `problemDecided` → `ok([])` (distinct events keep the log discriminable, no `bad-transition`); `ArtifactSource` gained `stakeholderCheck`/`chosenProblem`/`openModelAffectingHotSpots`; `absent-sc:<slug>` sweep key labels "Absent stakeholder: <name>".
