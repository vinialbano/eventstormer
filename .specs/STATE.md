...
- **Phase / Task**: Execute — Batches 1–5 done (T1–T41 + T40b). Next: Batch 6 (Phase 9 app rendering, T42–T46).
- **Branch**: `slice-4-hot-spots-close` (not pushed). Head `0e67303`. `pnpm check` + `pnpm build` green, **914 tests** (baseline 718). Domain + backend for F08/F09/F18 complete; only the Vue SPA + close ceremony + changeset remain.
...
- **Next step**: dispatch Batch 6 (Phase 9 app: hot-spot callouts + count on the wall, direct-flag affordance, resolution cards, pending drawer — T42–T46). Then B7 (T47–T51: `/impeccable` close-ceremony brief, in-dock close flow, E2E, `minor` changeset 0.5.0), then the Verifier.
