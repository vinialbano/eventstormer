...
- **PR #91 review fix (`aca9775`, 2026-09-07)**: B1 (BLOCK) — the AD-038 "effect already holds → `ok([])`" branch was missing from `decideInsertBetween` / `decideUnlinkCause`, both facilitator-reachable, so an AD-016 crash-window re-accept left the board mutated with the `Proposal` stream + F19 transcript stuck on `APPLY_FAILED`; fixed (both now return `ok([])` on the already-satisfied state) + `decide.test.ts` / `accept.test.ts:508`,`:530` convergence tests. W1 (WARN) — `labelsProposedThisTurn` built incrementally made the same-turn reword carve-out order-dependent; fixed with a pre-pass + a reword-first `map.test.ts` fixture. W3 — AD-035 / AD-038 bodies amended (named-field `intent` union; `Operation Rejected` not "Operation Application Failed"; un-relation convergence recorded). Deferred to #92: W2 (resolution-race losing-racer signal) + W5 (same-target reword lost-update).
- **PR #91 review round 3 (2026-09-07)**: doc WARNs reconciled (gate count `1154 → 1160`, diff range, live-smoke / issue-rescope bullets past-tensed). Testing gap closed — the `aca9775` seam convergence tests passed for the wrong reason (double `accept`; the first reaches `APPLIED`, the second is a route no-op), so reverting the B1 fix left them green. Replaced with a crash-window enumeration in `accept.test.ts` keyed on the `Intent` schema union — every relation / pivotal / reword kind, stuck-at-`ACCEPTED` → single `accept` → `APPLIED`, no second mutation; a new kind fails to typecheck without a fixture; verified to fail on the reverted fix. Latent edge (target withdrawn *during* the crash window → `APPLY_FAILED` on an applied `place`/`unplace`/`reword` proposal) needs a `BoardWriteModel` widen — tracked for Slice 6.
- **Original flags (context)**:
  ...
  3. T26 live smoke (`pnpm smoke:facilitation-schema`, FREL-04's manual half, out of CI per AD-027) **ran 2026-09-07 (`8de1dad`): PASS** — `research/research-aisdk.md` "SLICE 5" records the live result.
