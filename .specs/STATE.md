...
- **Verifier — look here**: the **scope opening question is never marked resolved on the Session stream** (`set-scope` writes only the Workshop; `Question Answered` is never appended for `Question Asked {kind:'scope'}`), so every `Close Session` sweeps it into a hot spot. The T50 E2E asserts this as current behaviour with a tolerant count. Decide whether scope-set should append `Question Answered` on the session or the sweep should skip `kind:'scope'` — either is a Phase-1/6/8 fix, out of Batch 7 scope.
