#!/usr/bin/env python3
"""
lessons.py — deterministic bookkeeping for the spec-driven-development lessons layer.
...
def _parse_date(s):
    try:
        ...
        # A malformed timestamp is treated as very old, not "now": falling back to
        # now() would make a stale candidate look freshly-seen and keep it alive
        # forever. Old-dating it lets _auto_prune drop the corrupt record instead.
        return _EPOCH
...
def _load(root):
    ...
    with open(path, "r", encoding="utf-8") as f:
        try:
            ...
            # A corrupt/partial store must fail loudly, not crash with a raw
            # traceback — otherwise even `init` (which routes through _load) can't
            # report why. The file is machine-owned; the fix is to inspect/remove it.
            sys.stderr.write(
                f"ERROR: {path} is not valid JSON ({e}).\n"
                "It may be a partial write. Inspect it, then remove it and re-run `init`.\n"
            )
            raise SystemExit(2)
...
def _save(root, data):
    ...
    path = _store_path(root)
    # Write to a temp file in the same dir, fsync, then atomically replace the
    # canonical store. A crash/kill/disk-full mid-write can only corrupt the temp
    # file — os.replace is atomic on POSIX, so lessons.json is never left empty or
    # partial (it is the source of truth; LESSONS.md is regenerated from it).
    tmp = path + ".tmp"
...
def main(argv=None):
    p = argparse.ArgumentParser(prog="lessons.py", description="Deterministic lessons bookkeeping for spec-driven-development.")
