...
15. **`ddd-strategic-design` reconciliation pass (2026-08-28).** Propagated the PRD F10 determinism
   pass (item 14 / `open-questions.md` #70) into the two artifacts that still described the retired
   Flow C. `context-map.md` — the **Session Facilitation → Derived Artifact Generation** row and
   Decision section: Flow C dropped; the edge now carries **two published reads** — the **session
   record** (`Session` stream, read by `Export Session Transcript`) and the **workshop record**
   (F18, read by `Export Model`); the unconfirmed term "session log" replaced by "session record"
   throughout; the stale `derived_from` digest for the DAG canvas refreshed.
...
| `context-map.md` (decided form) | draft, confirmed strategically | High | Every relationship reasoned through the U/D test and confirmed with the participant; the 2026-08-26 Question & Hot Spot Resolution collapse is adopted and reflected in the diagram/table; the Facilitation↔Capture relationship note names `Resolve Hot Spot` explicitly; the Domain Model Capture Design-Level session (2026-08-26) validated the seam without moving it; 2026-08-28 adoption pass added the **Session Facilitation → Derived Artifact Generation** edge and the Capture↔Facilitation apply-confirmation round-trip wording (#39, #51); 2026-08-28 reconciliation pass propagated the PRD F10 determinism change into that edge — Flow C dropped, two named reads (session record + workshop record), "session log" → "session record" (#72) |
...
| [bounded-contexts/session-facilitation/canvas.md](bounded-contexts/session-facilitation/canvas.md) | design-level | session-facilitation | draft | 2026-08-28 |
