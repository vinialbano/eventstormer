...
| Domain Model Capture | Core | The mechanism behind the product's central claim — "board and docs are the same object, no drift" — is the typed graph itself: stable ids, rename propagation, invariants across element/relation kinds | Build, best talent | PRD §1 ("a typed graph of records with stable identities"); §2 opportunity ("against drift") | High | `[confirmed]` |
