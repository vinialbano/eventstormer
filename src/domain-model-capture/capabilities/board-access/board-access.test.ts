...
describe('applyOperation — happy path', () => {
  it('appends the operation and returns the resulting building-block id + next position', () => {
    ...
    const result = applyOperation(deps, workshopId, captureOp('b_1', 'Loan recorded'))
...
describe('applyOperation — merits rejections bubble to the caller', () => {
  it('duplicate-id → Rejection', () => {
    const deps = depsFor(createMemoryEventStore())
    applyOperation(deps, workshopId, captureOp('b_1', 'first'))
    const result = applyOperation(deps, workshopId, captureOp('b_1', 'again'))
...
describe('applyOperation — a genuine two-accept race', () => {
  it('both operations apply with no APPLY_FAILED — the stale-position retry is internal', () => {
    ...
    const first = applyOperation(deps, workshopId, captureOp('b_1', 'Loan recorded'))
    const second = applyOperation(deps, workshopId, captureOp('b_2', 'Book returned'))

    expect(isOk(first) && isOk(second)).toBe(true)
    if (isOk(second)) expect(second.value.nextPosition).toBe(1)
    expect(readBuildingBlocks(depsFor(base), workshopId).map((block) => block.id)).toEqual(['b_1', 'b_2'])
  })
})
