...
const readProposal = (deps: AcceptProposalDeps, id: ProposalId): ProposalEvent[] =>
  deps.store.read(proposalStream(id)).map((row) => ProposalEvent.parse(row.operation))

const readSession = (deps: AcceptProposalDeps, id: SessionId): SessionEvent[] =>
  deps.store.read(sessionStream(id)).map((row) => SessionEvent.parse(row.operation))

const appendProposal = (deps: AcceptProposalDeps, id: ProposalId, events: ProposalEvent[]): void => {
 ...
 * idempotent 200. Called by `review-proposal/accept.ts`'s HTTP route and by
 * the stuck-`ACCEPTED` reconciliation sweep, so both re-drive through the
 * same path instead of two implementations that could drift. Lives in
 * `infrastructure/`, not a capability slice, because both callers need it —
 * capability slices may not import each other.
 */
export const acceptProposal = (deps: AcceptProposalDeps, id: ProposalId): Handled => {
