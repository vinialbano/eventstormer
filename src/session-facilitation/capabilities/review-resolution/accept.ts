 ...
 * `review-proposal/accept.ts`'s `acceptProposal`:
 *
 ...
 * `POST /resolutions/:id/accept` and by the stuck-`ACCEPTED` reconciliation
 * sweep, so both re-drive through the same path instead of two implementations
 * that could drift.
 */
export const acceptResolution = (deps: ReviewResolutionDeps, id: ResolutionId): Handled => {
